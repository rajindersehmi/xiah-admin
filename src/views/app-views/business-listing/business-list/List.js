...
const BusinessListingList = ({ mode }) => {
  ...
  const fetchAll = async (params) => {
    ...
    try {
      let res;
      if (mode === "all") res = await BusinessListingService.list(params);
      else res = await BusinessListingService.listClaims(params);
      ...
      async onOk() {
        try {
          ...
          if (selectedRows.length > 0)
            selectedRows.map((r) => {
              return formdata.append("id[]", r.id);
            });
          else {
  ...
  const tableColumns = [
    ...
    {
      title: "Category",
      dataIndex: "bussiness_category",
      render: (_, element) =>
        element?.bussiness_category?.length > 0
          ? element.bussiness_category.map((c) => (
              <Tag color={"blue"} className="d-block mb-1">
                {c?.name}
              </Tag>
            ))
          : null,
    ...
    {
      title: "User Clicks",
      dataIndex: "business_listing_analytic_count",
    },
    {
      title: "",
      dataIndex: "actions",
        ...
        <Flex alignItems="center" justifyContent="between" mobileFlex={false}>
          <Flex className="mb-1" mobileFlex={false} alignItems="center">
            <div className="mb-3 mr-md-3">
              <Radio.Group
                value={filter.is_claimed}
                buttonStyle="solid"
                onChange={(e) => _onFilterChange("is_claimed", e.target.value)}
              >
                <Radio.Button value={1}>Claimed</Radio.Button>
                <Radio.Button value={0}>Not Claimed</Radio.Button>
              </Radio.Group>
            </div>
