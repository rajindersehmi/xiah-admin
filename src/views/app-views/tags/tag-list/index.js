...
const TagList = () => {
  ...
  const fetchAll = async (params) => {
    ...
    try {
      const res = await TagService.list(params);
      setList(res.data);
  ...
  const addProduct = () => {
    history.push(`/app/tags/add-tag`);
  };

  const viewDetails = (row) => {
    history.push(`/app/tags/edit-tag/${row.id}`);
  };
    ...
    confirm({
      ...
      async onOk() {
        try {
          ...
          await TagService.delete(formdata);
    ...
    },
    {
      ...
      render: (_, record) => (
        <div className="d-flex">
          <AvatarStatus
            src={env.BASE_IMG_URL + "/tags/" + record.image}
            name={record.name}
          />
        </div>
      ),
