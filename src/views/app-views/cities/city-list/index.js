...
const AwardList = () => {
  ...
  const addProduct = () => {
    history.push(`/app/cities/add-city`);
  };

  const viewDetails = (row) => {
    history.push(`/app/cities/edit-city/${row.id}`);
  };
    ...
    confirm({
      ...
      async onOk() {
        try {
          ...
          await CityService.delete(formdata);
  ...
  const tableColumns = [
    ...
    },
    {
      ...
      dataIndex: "in_use",
      render: (_, elm) => (
        <Tag color={Number(elm?.in_use) === 1 ? "success" : "error"}>
          {Number(elm?.in_use) === 1 ? "Yes" : "NO"}
        </Tag>
      ),
    },
    {
      ...
      render: (_, elm) => (
        <Tag color={Number(elm?.is_terminated) === 1 ? "success" : "error"}>
          {Number(elm.is_terminated) === 1
            ? ("Yes", (elm.in_use = "Yes"))
            : "NO"}
        </Tag>
      ),
